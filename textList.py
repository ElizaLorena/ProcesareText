personalQuestions = [
    "What is your beautiful name?",
    "Tell me your name",
    "What is your partner's name?",
    "Cata is your name?",
    "Is Thomas your name?",
    "Tell me one of your favorite movie.",
    "Do you like your job?",
    "Do you have a boyfriend?",
    "Do you think that I am fat?",
    "What do you do?",
    "What do you have there?",
    "What is your opinion about Obama?",
    "What kind of drogs do you use?",
    "Who is your best friend?",
    "Who are you?",
    "Where are you from?",
    "where is your boyfriend?",
    "where do you leave?",
    "where are you leaving?",
    "Where is your house?",
    "When do you go home?",
    "When is your meeting?",
    "When are you turn into superman?",
    "Why do you work here?",
    "Why do you like your job?",
    "How much money do you have",
    "How many books do you read?",
    "How old are you?",
    "Where do you live?",
    "You are funny.",
    "Do you like your job?",
    "Do you have a boyfriend?",
    "Do you think that I am fat?",
    "Do you do that?",
    "Do you work?"]

infoAbout = [
    "What is a fork?",
    "What are sheep?",
    "What have been cooked in syrup and glazed to make the sweetmeat Marrons Glaces? ",
    "What does a numismatist study or collect?",
    "Who is Hitler?",
    "Who won six consecutive Wimbledon singles titles in the 1980s?",
    "Who created the UK TV spoof 007 character Basildon Bond?",
    "Who captained Jules Verne's submarine Nautilus? ",
    "Where was WW2 ?",
    "How many arms does a squid have?",
    "Which hills divide England from Scotland?",
    "Which American state is nearest to the former Soviet Union?",
    "In which year did Jordan won ...?"
]

textAnaphora = [
    "Who was Obama",
    "Is he black",
    "I think he is orange",
    "Me, Obama and Trump are great!",
    "Are we great?",
    "Neymar, Messi and Pogba have a great skils",
    "Their skills are indestructible",
    "Apple is a great fruit.",
    "What is your opinion about it?"
]

differenceExamples = [
    "Who is the best? Obama or Bush?",
    "Who has more apple juice? X, Y or Z?",
    "Who is older? Obama, Putin, Merkel or Churchill?",
    "How many km are between Iasi and Suceava"
]

yesOrNo = [
    "Are they American?",
    "Is New York nice?",
    "Was he pepsi?",

    "Are they visiting Paris",
    "Has Nancy been working all night long?",

    "Do you like apples?",
    "Do they go to a high school?",

    "Does Nancy read a lot?",
    "Does he hate basketball?",

    "Did John discover the truth?",
    "Did the chicken cross the road?"
]

opinionQuestions = [

	"When do you think school will begin?",
	"What do you think about me?",
    "What is your opinion about school?",
	
]